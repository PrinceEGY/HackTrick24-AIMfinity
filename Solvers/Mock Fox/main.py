...
carrier_image = np.array(imread("D:\HackTrick\Sol\HackTrick24\Banana.jpg"))
secret_message = "THE ABC MERCY WELL B"

@app.route('/fox/start', methods=['POST'])
def start_game():
    team_id = request.json.get('teamId')
    return jsonify({"msg": secret_message, "carrier_image": carrier_image.tolist()}), 200


data = pd.read_csv("D:\HackTrick\Repo\HackTrick-AIMfinity\Riddles\ml_easy_sample_example\series_data.csv")
...
dict = \
    {
        ...
        "ml_easy":{"timestamp":data["timestamp"].tolist(),
                   "visits":data["visits"].tolist()},
        "ml_medium":[-20.73948417,0.061261011],
        "problem_solving_easy":(["pharaoh","sphinx","pharaoh","pharaoh","nile", "sphinx","pyramid","pharaoh",
        "sphinx","sphinx"],3),
...
def solve_riddle():
    ...
    total_budget = 1000
    status = "success"
    return jsonify({"budget increase": budget_increase, "total budget": total_budget, "status": status}), 200
...
def end_game():
    ...
    score = 10
    new_highscore = True
    return f"Game ended successfully with a score of {score}. {'New Highscore reached!' if new_highscore else ''}", 200
